...
using Studio23.SS2.IngameAchievements.UI;
...
namespace Studio23.SS2.IngameAchievements.Core
{
	public class AchivementsManager : MonoBehaviour
	{
		...
		void Awake()
		{
			...
			if (File.Exists(
					SavePath)) //available for locally only -> need to give save path to steam cloud to work on cloud
			{
				LoadAchievements();
			}
			else
		...
		public void TestDummyUnlock(string name)
		{
			...
			toastCard.GetComponent<AchievementPopupAnimation>().EnablePopUp(achievement.UnlockedIcon,
